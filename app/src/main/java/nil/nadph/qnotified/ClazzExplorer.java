...
public class ClazzExplorer{
	...
	public TextView newTv(){
		...
		tv.setOnLongClickListener(new OnLongClickListener(){
				...
				public boolean onLongClick(View v){
					...
					Toast.makeText(v.getContext(),"已复制内容",0).show();
					return true;
				}
			});
			...
			if(clazz.isArray()){
				...
				for(i=0;i<len;i++){
					...
					for(i=0;i<fs.length;i++){
						...
						}catch(Exception e){t_tmp=e;}
						if(first)tmp="<small><font color='#A0A0FF'>"+fd+"</small></font><br/><b> "+Html.escapeHtml(fs[i].getName())+" = "+Html.escapeHtml(t_tmp==null?"null":nomorethan100(t_tmp))+"</b>";
						else tmp="<small><font color='#A0A0FF'>"+fd+"</small></font><br/> "+Html.escapeHtml(fs[i].getName())+" = "+Html.escapeHtml(t_tmp==null?"null":nomorethan100(t_tmp));
						if(num++<tvs.size()){
							tv=tvs.get(num-1);
						}else tv=newTv();
						tv.setTag(fs[i]);
					...
					for(i=0;i<ms.length;i++){
						...
						if(first)tmp="<small><font color='#FFA0A0'>"+fd+"</small></font><br/><b> "+Html.escapeHtml(ms[i].getName())+"("+Html.escapeHtml(tmp)+")</b>";
						else tmp="<small><font color='#FFA0A0'>"+fd+"</small></font><br/> "+Html.escapeHtml(ms[i].getName())+"("+Html.escapeHtml(tmp)+")";
						if(num++<tvs.size()){
							tv=tvs.get(num-1);
						}else tv=newTv();
						tv.setTag(ms[i]);
	...
	public static Activity getCurrentActivity(){
		...
		}catch(ClassNotFoundException e){ e.printStackTrace(); }catch(InvocationTargetException e){ e.printStackTrace(); }catch(NoSuchMethodException e){ e.printStackTrace(); }catch(NoSuchFieldException e){ e.printStackTrace(); }catch(IllegalAccessException e){ e.printStackTrace(); }
		return null; 
	} 
}
