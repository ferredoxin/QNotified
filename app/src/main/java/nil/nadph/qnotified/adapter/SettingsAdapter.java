...
public class SettingsAdapter implements ActivityAdapter {
    ...
    public void doOnPostCreate(Bundle savedInstanceState) throws Throwable {
        ...
        if(!Utils.isTim(self))ll.addView(newListItemSwitchConfigNext(self, "隐藏小程序入口", "隐藏消息列表下拉出现的小程序列表", qn_hide_msg_list_miniapp, false));
        if(!Utils.isTim(self))ll.addView(newListItemSwitchConfigNext(self, "隐藏分组下方入口", "隐藏分组列表最下方的历史好友按钮", qn_hide_ex_entry_group, false));
